...
@Component
@Primary
public class CachedSubCommentService extends CachedServiceHelper<SubComment> implements SubCommentService {

    private final SubCommentService subCommentService;

    public CachedSubCommentService(RedisTemplate<String, SubComment> subCommentRedisTemplate, @Qualifier("subCommentServiceImpl") SubCommentService subCommentService) {
    ...
    public SubComment save(CreateSubCommentRequest request) {
        ...
        var cached = super.getOrLoad("subComment:" + subComment.getId(), () -> subComment, Duration.ofHours(2));
    ...
    public SubComment findById(long id) {
        var cached = super.getOrLoad("subComment:" + id, () -> subCommentService.findById(id), Duration.ofHours(2));
        return cached.orElseThrow(() -> new SubCommentNotFoundException("Sub comment not found with a id: " + id));
    }
    ...
    public List<SubComment> findAllByCommentId(long commentId, SortType sortType, int limit, int offset) {
        ...
        return super.getListOrLoad(redisKey, () -> subCommentService.findAllByCommentId(commentId, sortType, limit,offset), limit, offset, Duration.ofHours(2));
    }
