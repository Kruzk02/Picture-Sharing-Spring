...
class PinServiceImplTest {
    ...
    void update_ShouldUpdatePin_WhenValidRequestAndMatchingUser() {
        ...
        try (
            ...
            mediaManagerMock.when(() -> MediaManager.generateUniqueFilename("new-file.jpg"))
                    .thenReturn(newFilename);
            mediaManagerMock.when(() -> MediaManager.getFileExtension("new-file.jpg"))
                    .thenReturn(newExtension);
            mediaManagerMock.when(() -> MediaManager.getFileExtension("old-file.png"))
                    .thenReturn("png");

            fileManagerMock.when(() -> FileManager.delete(Mockito.anyString(), Mockito.anyString()))
                    .thenReturn(CompletableFuture.completedFuture(null));
            fileManagerMock.when(() -> FileManager.save(Mockito.any(), Mockito.anyString(), Mockito.anyString()))
                    .thenReturn(CompletableFuture.completedFuture(null));
