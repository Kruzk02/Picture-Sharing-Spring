...
class UserServiceImplTest {
    ...
    void setUp() {
        ...
        Media media = Media.builder().id(1L).mediaType(MediaType.IMAGE).url("default_profile_picture.png").build();
    ...
    void register_shouldRegisterUserSuccessfully() {
        ...
        Mockito.when(passwordEncoder.encode(registerUserRequest.password())).thenReturn("encodedPassword");

        Mockito.when(roleDao.findByName("ROLE_USER")).thenReturn(role);

        Mockito.when(userDao.register(Mockito.argThat(u ->
        ...
        Mockito.when(verificationTokenService.generateVerificationToken(Mockito.eq(user))).thenReturn(token);
    ...
    void login_shouldReturnUser_whenCredentialsAreValid() {
        var token = new UsernamePasswordAuthenticationToken(loginUserRequest.username(), loginUserRequest.password());
    ...
    void update_shouldUpdateUserSuccessfully() {
        ...
        Mockito.when(userDao.update(Mockito.argThat(user ->
        ...
        Mockito.verify(userDao).update(Mockito.argThat(user ->
                user.getUsername().equals("newUsername") &&
                user.getEmail().equals("new@example.com") &&
                user.getPassword().equals("encodedNewPassword") &&
                user.getBio().equals("Updated bio") &&
