...
class SubCommentDaoImplTest {
    ...
    void findAllByCommentId_shouldReturnListOfSubComment_whenCommentExists() {
        when(jdbcTemplate.query(
                eq("SELECT sc.id AS sc_id, sc.content AS sc_content, sc.create_at as sc_create_at, " +
    ...
    void findById_shouldReturnSubComment_whenSubCommentExists() {
        ...
        verify(jdbcTemplate).queryForObject(eq("SELECT sc.id as sc_id, sc.content AS sc_content, sc.create_at AS sc_create_at, " +
                        "sc.media_id AS sc_media_id, " +
                        "sc.comment_id AS sc_comment_id, " +
                        "u.id AS user_id, " +
                        "u.username AS user_username, " +
    ...
    void findById_shouldThrowException_whenSubCommentDoesNotExists() {
        when(jdbcTemplate.queryForObject(anyString(), any(SubCommentRowMapper.class), eq(1L))).thenThrow(new EmptyResultDataAccessException(1));
