...
class BoardDaoImplTest {
    ...
    void addPinToBoard_shouldInsertPinToBoard() {
        ...
        Mockito.verify(jdbcTemplate).update("INSERT INTO board_pin (board_id, pin_id) VALUES(?,?)", 1L, 1L);
    }
    ...
    void deletePinFromBoard() {
        ...
        Mockito.verify(jdbcTemplate).update("DELETE FROM board_pin WHERE board_id = ? AND pin_id = ?", 1L, 1L);
    }

    @Test
    void findById_shouldReturnBoard_whenBoardExists() {
        Mockito.when(jdbcTemplate.query(
                Mockito.eq("SELECT b.id AS board_id, b.board_name, b.create_at, " +
    ...
    void findById_shouldThrowException_whenBoardDoesNotExists() {
        Mockito.when(jdbcTemplate.query(Mockito.anyString(), Mockito.any(BoardResultSetExtractor.class), Mockito.eq(1L))).thenThrow(new EmptyResultDataAccessException(1));
    ...
    void deleteById_shouldDeleteBoard_whenBoardExists() {
        Mockito.when(jdbcTemplate.update(
                Mockito.eq("DELETE FROM boards WHERE id = ?"),
                Mockito.eq(1L)
        )).thenReturn(1);
