...
class UserDaoImplTest {
  ...
  void register_ShouldInsertUserAndAssignRole() {
    ...
    Mockito.verify(jdbcTemplate)
        .batchUpdate(
            Mockito.eq("INSERT INTO users_roles (user_id, role_id) VALUES (?, ?)"),
  ...
  void login_shouldReturnUser_whenUserExists() {
    when(jdbcTemplate.queryForObject(
            eq(
                "SELECT u.id, u.username, u.email, u.password, u.enable, roles.id as role_id, roles.name as role_name "
                    + "FROM users u "
  ...
  void login_shouldReturnNull_whenUserDoesNotExists() {
    when(jdbcTemplate.queryForObject(anyString(), any(UserRowMapper.class), eq(user.getUsername())))
  ...
  void findUserById_shouldThrowException_whenUserDoesNotExists() {
    when(jdbcTemplate.queryForObject(anyString(), any(UserRowMapper.class), eq(user.getId())))
        .thenThrow(new EmptyResultDataAccessException(1));
  ...
  void findUserByEmail_shouldThrowException_whenUserDoesNotExists() {
    when(jdbcTemplate.queryForObject(anyString(), any(UserRowMapper.class), eq(user.getEmail())))
        .thenThrow(new EmptyResultDataAccessException(1));
    var result = userDao.findUserByEmail(user.getEmail());
    assertNull(result);
  }
  ...
  void findFullUserByUsername_shouldReturnUser_whenUserExists() {
    when(jdbcTemplate.queryForObject(
            eq(
                "SELECT id, username, email, password, bio, media_id, enable, gender FROM users WHERE username = ?"),
            any(UserRowMapper.class),
            eq(user.getUsername())))
        .thenReturn(user);
  ...
  void findPasswordNRoleByUsername_shouldReturnUser_whenUserExists() {
    when(jdbcTemplate.query(
            eq(
                "SELECT u.id AS user_id, u.username AS user_username, u.password AS user_password, r.id AS role_id, r.name AS role_name "
                    + "FROM users u "
  ...
  void findPasswordNRoleByUsername_shouldThrowException_whenUserDoesNotExists() {
    when(jdbcTemplate.query(anyString(), any(ResultSetExtractor.class), eq(user.getUsername())))
        .thenThrow(new EmptyResultDataAccessException(1));
    assertThrows(
        UserNotFoundException.class, () -> userDao.findPasswordNRoleByUsername(user.getUsername()));
  }

  @Test
  void checkAccountVerifyById_shouldReturnTrue_whenUserDoesNotEnable() {
    when(jdbcTemplate.queryForObject(
            eq("SELECT enable FROM users WHERE id = ?"), any(RowMapper.class), eq(1L)))
        .thenReturn(true);
    ...
    verify(jdbcTemplate)
        .queryForObject(eq("SELECT enable FROM users WHERE id = ?"), any(RowMapper.class), eq(1L));
  }

  @Test
  void checkAccountVerifyById_shouldThrowException_whenUserDoesNotExists() {
    when(jdbcTemplate.queryForObject(anyString(), any(RowMapper.class), eq(user.getId())))
        .thenThrow(new EmptyResultDataAccessException(1));
