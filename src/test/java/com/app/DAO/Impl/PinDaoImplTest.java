...
class PinDaoImplTest {
    ...
    void setUp() {
        pin = Pin.builder()
                ...
                .userId(1L)
                .mediaId(1L)
                .hashtags(List.of(Hashtag.builder()
                        .id(1L)
                        .tag("tag")
                        .build()))
                .build();
    ...
    void getAllPins_shouldReturnListOfPins() {
        Mockito.when(jdbcTemplate.query(
                Mockito.eq("SELECT id, user_id, media_id, created_at FROM pins ORDER BY created_at DESC LIMIT ? OFFSET ?"),
    ...
    void getAllPinsByHashtag_shouldReturnListOfPins() {
        Mockito.when(jdbcTemplate.query(
                Mockito.eq( "SELECT p.id, p.user_id, p.media_id, p.created_at " +
                        "FROM pins p " +
    ...
    void save_shouldInsertPinAndAssignHashTag() {
        ...
        Mockito.verify(jdbcTemplate).batchUpdate(Mockito.eq("INSERT INTO hashtags_pins(hashtag_id, pin_id) VALUES(?, ?)"), Mockito.any(BatchPreparedStatementSetter.class));
    }
    ...
    void findById_shouldThrowException_whenPinDoesNotExists() {
        Mockito.when(jdbcTemplate.queryForObject(Mockito.anyString(), Mockito.any(PinRowMapper.class), Mockito.eq(1L))).thenThrow(new EmptyResultDataAccessException(1));
    ...
    void findById_shouldReturnPinWithDetails_whenPinExists() {
        Mockito.when(jdbcTemplate.query(
                Mockito.eq("SELECT p.id AS pin_id, p.user_id, p.description, p.media_id, p.created_at, " +
    ...
    void findById_shouldThrowException_whenPinWithDetailsDoesNotExists() {
        Mockito.when(jdbcTemplate.query(Mockito.anyString(), Mockito.any(PinRSE.class), Mockito.eq(1L))).thenThrow(new EmptyResultDataAccessException(1));
    ...
    void findPinByUserId_shouldReturnListOfPin_whenUserExists() {
        Mockito.when(jdbcTemplate.query(
                Mockito.eq("SELECT id, user_id, media_id, created_at FROM pins WHERE user_id = ? ORDER BY created_at DESC limit ? offset ?"),
                Mockito.any(PinRowMapper.class),
                Mockito.eq(1L),
                Mockito.eq(10),
                Mockito.eq(0)
    ...
    void findPinByUserId_shouldThrowException_whenUserDoesNotExists() {
        Mockito.when(jdbcTemplate.query(Mockito.anyString(), Mockito.any(PinRowMapper.class), Mockito.eq(1L), Mockito.eq(10), Mockito.eq(0))).thenThrow(new EmptyResultDataAccessException(1));
    ...
    void deleteById_shouldDeletePin_whenPinExists() {
        Mockito.when(jdbcTemplate.update(
                Mockito.eq("DELETE FROM pins WHERE id = ?"),
                Mockito.eq(1L)
        )).thenReturn(1);
    ...
    void deleteById_shouldThrowException_whenPinDoesNotExists() {
        Mockito.when(jdbcTemplate.update(Mockito.anyString(), Mockito.eq(1L))).thenThrow(new EmptyResultDataAccessException(1));
